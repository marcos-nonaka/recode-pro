...
class DestinoEdit extends Component {
    ...
    async componentDidMount() {
        if (this.props.match.params.id !== 'new') {
            const destino = await (await fetch(`/cadastro-destino/${this.props.match.params.id}`)).json();
            this.setState({ item: destino });
        }
    }
    ...
    async handleSubmit(event) {
        ...
        await fetch('/cadastro-destino' + (item.id ? '/' + item.id : ''), {
        ...
        this.props.history.push('/cadastro-destino');
    }

    render() {
        const { item } = this.state;
        const title = <h2>{item.id ? 'Editar Destino' : 'Adicionar Destino'}</h2>;
            ...
            <Container>
                ...
                <Form onSubmit={this.handleSubmit}>
                    ...
                    <FormGroup>
                        ...
                        <Button color="secondary" tag={Link} to="/cadastro-destino">Cancelar</Button>
                    </FormGroup>
                </Form>
            </Container>
            <AdminFooter />
...
export default withRouter(DestinoEdit);